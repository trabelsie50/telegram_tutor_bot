...
def _extract_json_object(raw_text: str) -> Dict[str, Any]:
    ...
    # حل احتياطي: البحث عن أول كائن JSON صالح داخل النص
...
class RAGEngine:
    ...
    def retrieve_for_quiz(
        ...
        if not results:
            # لا نتائج بفلتر lesson_name الدقيق: نحاول بحثاً دلالياً أوسع بدون فلتر الدرس
            results = self.retrieve_relevant(
                query=lesson_name,
                top_k=top_k,
                subject_filter=subject,
    ...
    def explain_lesson(self, lesson_name: str, subject: str) -> str:
        """يشرح درساً معيناً بالاستناد فقط إلى النصوص المخزنة في RAG لهذا الدرس/المادة."""
    ...
    def generate_quiz(
        ...
        """يولّد اختباراً ذكياً (JSON منظّم) بالاستناد فقط إلى نص الدرس المخزَّن في RAG."""
        lesson_text = self.retrieve_for_quiz(lesson_name, subject, top_k=6)
        if not lesson_text.strip():
            return {"questions": []}
        ...
        # تحقق دفاعي من شكل كل سؤال حتى لا ينهار الكود إذا رد النموذج بشكل ناقص
    ...
    def evaluate_answer(
        ...
        """يقيّم إجابة التلميذ بالذكاء الاصطناعي (تسامح مع الصياغة المختلفة وأخطاء النطق الصوتي)."""
        ...
        if "is_correct" not in parsed:
            # حل احتياطي إذا فشل تحليل JSON: تطابق كلمات بسيط
            is_correct = any(
