...
class Settings:
    ...
    # استبدل OpenAI بالكامل: للشرح، الاختبار، تقييم الإجابات، والـ embeddings.
    gemini_api_key: str = field(default_factory=lambda: os.getenv("GEMINI_API_KEY", ""))

    # ── نموذج Gemini للمحادثة/الشرح ─────────────────────────────
    gemini_model: str = field(default_factory=lambda: os.getenv("GEMINI_MODEL", "gemini-2.5-flash"))

    # ── نموذج Gemini لتوليد embeddings (بديل sentence-transformers المحلي) ──
    ...
    # ── محرك OCR الافتراضي (Tesseract فقط، easyocr أُزيل نهائياً) ─
