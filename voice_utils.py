...
# ملاحظة: مكتبة whisper (وتسحب معها PyTorch) تُستورَد بشكل كسول داخل
# _get_whisper_model() فقط، وليس هنا في أعلى الملف. كان استيرادها هنا يحمّل
# PyTorch في الذاكرة فوراً عند بدء تشغيل البوت — حتى لو ما بعث حتى تلميذ واحد
# رسالة صوتية — وهذا كان من أسباب تجاوز حد الذاكرة (512MB) على Render.
...
def _get_whisper_model() -> Any:
    ...
    ويستورد مكتبة whisper/PyTorch هنا فقط عند أول استدعاء فعلي).
    ...
    if _whisper_model is None:
        import whisper  # استيراد كسول: PyTorch يُحمَّل هنا فقط، عند أول رسالة صوتية
...
def get_audio_info(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError("تعذّر تحليل معلومات الملف الصوتي كـ JSON.")
...
# ──────────────────────────────────────────────
# دوال المستوى العالي — الربط الفعلي مع البوت (جديد)
# ──────────────────────────────────────────────

def transcribe_telegram_voice(input_audio_path: str, language: str = DEFAULT_LANGUAGE) -> str:
    """يحوّل ملف صوتي (منزَّل من تيليغرام، أي صيغة) إلى نص باستخدام Whisper المحلي.

    هذه هي الدالة التي كانت مفقودة سابقاً: كل أدوات المعالجة أعلاه كانت
    موجودة لكن لا شيء يستدعي whisper.transcribe() فعلياً على رسالة صوت واردة.

    Args:
        input_audio_path: مسار الملف الصوتي الأصلي (OGG من تيليغرام عادة).
        language: كود اللغة المتوقعة ("ar", "fr", "en").

    Returns:
        النص المُفرَّغ (قد يكون فارغاً إذا لم يُفهم الصوت).
    """
    wav_path: Optional[str] = None
    try:
        wav_path = _convert_audio_format(input_audio_path)
        model = _get_whisper_model()
        result = model.transcribe(wav_path, language=language)
        return (result.get("text") or "").strip()
    finally:
        if wav_path and wav_path != input_audio_path and os.path.exists(wav_path):
            try:
                os.remove(wav_path)
            except OSError:
                pass
...
def text_to_speech(
    ...
    """يحوّل نصاً إلى ملف صوتي MP3 باستخدام gTTS (كانت مستوردة وغير مستعملة سابقاً).

    Args:
        text: النص المراد تحويله لصوت.
        lang: كود اللغة ("ar", "fr", أي شيء آخر يُعامل كإنجليزية).
        output_path: مسار ملف الإخراج (اختياري، افتراضياً في المجلد المؤقت).

    Returns:
        مسار ملف MP3 الناتج. المتصل مسؤول عن حذفه بعد الإرسال.
    """
    if not text or not text.strip():
        raise ValueError("النص المطلوب تحويله لصوت فارغ.")

    if output_path is None:
        output_path = os.path.join(
            tempfile.gettempdir(), f"tts_{os.getpid()}_{int(time.time())}.mp3"
        )

    gtts_lang = "ar" if lang == "ar" else ("fr" if lang == "fr" else "en")
